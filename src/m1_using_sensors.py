...
def run_test_go_straight_until_black():
    ...
    print()
    print('Test 1:')
    print('After the beep, I will go forward until I read black.')
    input('Press the ENTER key to continue.')
    beeper.beep()
...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_sensor(self, inches, speed):
        ...
        while True:
            ...
            current = math.fabs(self.left_motor.get_position())
            distance += (1.3 * math.pi / 180) * current
            if distance >= inches:
               self.stop()
               break
